...
@Component
public class DataSourceBasedMultiTenantConnectionProviderImpl extends AbstractDataSourceBasedMultiTenantConnectionProviderImpl<ClientDatabase> {
...
//        loadTenantsFromMasterDb();
    ...
    private void loadTenantsFromMasterDb() {
        try (Connection conn = masterDataSource.getConnection()) {
            ResultSet rs = conn.createStatement().executeQuery("SELECT tenant_id, db_url, db_user, db_password FROM tenants");
            while (rs.next()) {
